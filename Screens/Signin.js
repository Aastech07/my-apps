...
const loginUser = async () => {
  try {
    const {data} = await axios.post(
      "http://15.206.74.132:3000/api/login",
      {
        phone:number,
      }
    );
    console.log("OTP GET successful:", data.otp);
  ...
  return (
    ...
    >
      ...
      <View style={{ top: responsiveHeight(4) }}>
        ...
        <View style={{}}>
          <View style={{ top: responsiveHeight(42),left:responsiveWidth(3) }}>
            <Text style={{ fontSize: 16 }}>Don't have an account?</Text>
            <Text style={{ left: 170, bottom: 19, color: "#1977F3" }} onPress={()=>navigation.navigate('SignUp')}>
              Sign up
            </Text>
          </View>
        </View>
