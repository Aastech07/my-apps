...
const FurnitureDetail = () => {
  ...
  const POSTPRO = async () => {
    ...
    try {
      // const { data } = await axios.post(`${api}/furniture`, {
      //   profileId: profileid,
      //   furnitureType: brand,
      //   address: address,
      //   landmark: landmark,
  ...
  const POSTPROS = async () => {
    try {
      const { data } = await axios.put(`${api}/furniture/${Value._id}`, {
        furnitureType: brand,
        address: address,
        landmark: landmark,
        used: used,
  ...
  useEffect(() => {
    const getproperties = async () => {
      const { data } = await axios.get(`${api}/furniture/${Value._id}`);
      setBrand(data.furnitureType);
  ...
  return (
    <>
      ...
      <Alerts
        ...
        desc={"The Request For The Furniture Has Been Sent Successfully!"}
      />
    </>
  );
};
